...
const SqlString = __nccwpck_require__(4386);
...
exports.__defineGetter__(
  ...
  () => (__nccwpck_require__(2767).createConnection)
);

exports.__defineGetter__(
  'createPoolPromise',
  () => (__nccwpck_require__(2767).createPool)
);

exports.__defineGetter__(
  'createPoolClusterPromise',
  () => (__nccwpck_require__(2767).createPoolCluster)
);
...
class Connection extends EventEmitter {
  ...
  promise(promiseImpl) {
    const PromiseConnection = (__nccwpck_require__(2767).PromiseConnection);
...
class Handshake {
  ...
  setScrambleData(cb) {
    (__nccwpck_require__(6113).randomBytes)(20, (err, data) => {
      if (err) {
        cb(err);
        return;
      }
...
const AuthSwitchResponse = __nccwpck_require__(1489);
...
const NativeBuffer = (__nccwpck_require__(4300).Buffer);
...
class Pool extends EventEmitter {
  ...
  promise(promiseImpl) {
    const PromisePool = (__nccwpck_require__(2767).PromisePool);
...
class PoolConnection extends Connection {
  ...
  promise(promiseImpl) {
    const PromisePoolConnection = (__nccwpck_require__(2767).PromisePoolConnection);
...
module.exports = __nccwpck_require__(6797);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const promise_1 = __nccwpck_require__(2767);
const gh_action_i_o_1 = __nccwpck_require__(3115);
async function main() {
    const ghActionIO = new gh_action_i_o_1.GhActionIO();
    try {
        const connection = await (0, promise_1.createConnection)(ghActionIO.getConnectionSettings());
        await connection.beginTransaction();
        const results = await Promise.all(ghActionIO.getQueries().map(async (query) => {
